...
namespace MVVM_Base.ViewModel
{
    public partial class vmLinear : ObservableObject, IViewModel
    {
        ...
        private async Task Calculate(string mode)
        {
            ...
            try
            {
                await CalculateCoreAsync(mode, _calculateCts.Token);
            }
        ...
        private async Task CalculateCoreAsync(string mode, CancellationToken token)
        {
            ...
            foreach (var swValue in swValueList) 
            {
                ...
                if(res == "failed")
                {
                    // TODO UI有効化・無効化
                    canTransitOther = true;
                    //await messageService.ShowMessage("Failed to OverShoot");
                ...
                await WaitForDispose(waitTime, token);
        ...
        /// <returns></returns>
        private async Task WaitForDispose(int waitTime, CancellationToken token)
        {
            var waitOS = new HighPrecisionDelay();
            await waitOS.WaitAsync(waitTime, token);
        }
        ...
        private async Task<string> GetAndCalInitialVO(int index, CancellationToken token)
        {
            ...
            {
                // 上位バイト
                var result = await CommMFCAsyncType3("ER", "FE27", token);
                ...
                var upper = result.Substring(3, 2);
                var upperVal = Convert.ToInt32(upper, 16);

                // 下位バイト
                result = await CommMFCAsyncType3("ER", "FE26", token);
                if(result == "failed")
                {
                    return "failed";
                }
                ...
                var lower = result.Substring(3, 2);
        ...
        /// 解の二分探索
        /// </summary>
        private double BinarySearch(double target, Vector<double> coeffs)
        {
            double low = 0d;
            double high = 1d;
            double mid = 0d;
